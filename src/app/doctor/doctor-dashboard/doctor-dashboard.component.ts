...
@Component({
  ...
  imports: [CommonModule, NgChartsModule, LoadingComponent],
})
export class DoctorDashboardComponent implements OnInit {
          ...
          this.doctorDataService.getDoctorById(currentUser.id).subscribe({
            next: (response) => {
              ...
              if (response && response.user && response.user.name) {
                ...
                console.log(
                  'Doctor name successfully fetched from DB:',
                  this.doctorName,
                );
              }
            ...
            error: (error) => {
              ...
              if (currentUser.name) {
                ...
                console.log(
                  'Using fallback doctor name from auth:',
                  this.doctorName,
                );
              }
      ...
      try {
        ...
        if (currentUser && currentUser.id) {
          ...
          this.doctorDataService.getDoctorById(currentUser.id).subscribe({
            next: (doctorResponse) => {
              console.log(
                'Doctor response for getting doctorId:',
                doctorResponse,
              );
              ...
              if (doctorResponse && doctorResponse.id) {
                ...
                this.doctorDataService
                  .getPatientsByDoctorId(doctorResponse.id)
                  .subscribe({
                    next: (response) => {
            ...
            error: (error) => {
              console.error(
                'Error fetching doctor profile for patients:',
                error,
              );
              this.isLoadingPatients = false;
              this.totalPatients = 0;
  ...
  private loadTodayAppointments(): void {
    ...
    this.todayPatients = [];
    console.log(
      'Starting to load today appointments for doctor:',
    ...
    this.doctorDataService
      ...
      .subscribe({
        next: (response) => {
          ...
          console.log(
            `Total appointments count in response: ${response.appointments.length}`,
          );
          ...
          const todaysAppointments = response.appointments.filter(
            (apt: any) => {
          ...
          console.log(
            'Total today appointments after filtering:',
            this.totalTodayAppointments,
          );
          ...
          console.log(
            `Total unique patients today: ${this.totalTodayPatients}`,
          );
          console.log(
            `Total appointments today: ${this.totalTodayAppointments}`,
          );
          this.isLoadingAppointments = false;
        },
        error: (error) => {
  ...
  private loadNextAppointments(): void {
    ...
    console.log(
      'Starting to load next appointment for doctor:',
      this.currentDoctorId,
    );
        ...
        next: (response: any) => {
          ...
          const scheduledAppointments = response.appointments
            .filter((apt: any) => {
              ...
              console.log(
                `Appointment: ${apt.patientName}, Status: ${apt.status}, IsScheduled: ${isScheduled}`,
              );
              return isScheduled;
            })
            .sort((a: any, b: any) => {
          ...
          console.log(
            'Scheduled appointments for current doctor:',
          ...
          if (scheduledAppointments.length > 0) {
            ...
            this.nextPatient = {
              ...
              appointmentStatus:
                nextApt.status === 1 ? 'Completed' : 'Scheduled',
  ...
  private loadTotalAppointments(): void {
    ...
    console.log(
      'Starting to load total appointments for doctor:',
      this.currentDoctorId,
    );
  ...
  private generateCalendar(): void {
    ...
    this.firstDayOfMonth =
      this.firstDayOfMonth === 0 ? 6 : this.firstDayOfMonth - 1;
